...
public enum IteratorType
{
    ...
    /// It also includes newly inserted segments after any MoveSegmentZero.
    ...
    /// <summary>
    /// Snapshot iterator initiates MoveSegmentZero first when it is created.
    ...
    /// <summary>
    /// It is like Snapshot iterator but does not initiate MoveSegmentZero.
    /// It fetches all available read-only segments.
    /// Since it does not contain the mutable segment
    /// the iterator is consistent across its lifetime.
    /// It does not see new writes to the database.
    /// You may manually call MoveSegmentZero before creating the iterator,
    ...
    /// It does not include newly inserted segments after a new MoveSegmentZero event.
