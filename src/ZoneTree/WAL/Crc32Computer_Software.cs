...
public sealed class Crc32Computer_Software
{
    ...
    public static uint Compute(uint crc, ulong data)
    {
        unsafe
        {
            return UpdateScalar(crc, new ReadOnlySpan<byte>(&data, sizeof(ulong)));
        }
    }
    ...
    public static uint Compute(uint crc, uint data)
    {
        unsafe
        {
            return UpdateScalar(crc, new ReadOnlySpan<byte>(&data, sizeof(uint)));
        }
    }
    ...
    public static uint Compute(uint crc, int data)
    {
        unsafe
        {
            return UpdateScalar(crc, new ReadOnlySpan<byte>(&data, sizeof(int)));
        }
    }
    ...
    // See: https://github.com/dotnet/runtime/blob/main/src/libraries/System.IO.Hashing/src/System/IO/Hashing/Crc32.cs#L198-L204
    ...
    /// </remarks>
    static uint[] _crcLookupTable;
    static ReadOnlySpan<uint> CrcLookup => _crcLookupTable ??= new uint[]
    {
        // See: https://github.com/dotnet/runtime/blob/main/src/libraries/System.IO.Hashing/src/System/IO/Hashing/Crc32.Table.cs
        //
